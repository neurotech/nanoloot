...
NanoLootWidgetsGridviewMixin = {}
function NanoLootWidgetsGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
...
function NanoLootWidgetsGridviewMixin:InitFramePool(type, template)
    self.framePool = CreateFramePool(type, self.scrollChild, template);
end

function NanoLootWidgetsGridviewMixin:SetMinMaxSize(min, max)
    self.itemMinWidth = min;
    self.itemMaxWidth = max;
end

function NanoLootWidgetsGridviewMixin:Insert(info)
...
function NanoLootWidgetsGridviewMixin:RemoveFrame(frame)
    local key;
...
function NanoLootWidgetsGridviewMixin:InsertTable(tbl)

end

function NanoLootWidgetsGridviewMixin:Flush()
    self.data = {}
    for k, f in ipairs(self.frames) do
        if f.ResetDataBinding then
            f:ResetDataBinding()
...
function NanoLootWidgetsGridviewMixin:GetItemSize()
...
function NanoLootWidgetsGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function NanoLootWidgetsGridviewMixin:GetFrames()
    return self.frames;
end
